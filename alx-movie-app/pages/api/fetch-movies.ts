  ...
  if (request.method === "POST") {
    ...
    const resp = await fetch(
      `https://moviesdatabase.p.rapidapi.com/titles?year=${
        year || date.getFullYear() // Use the provided year or default to the current year
      }&sort=year.decr&limit=12&page=${page}&${genre && `genre=${genre}`}`, // Add the genre filter if provided
      {
        headers: {
          ...
          "x-rapidapi-key": `${process.env.MOVIE_API_KEY}`, // API key retrieved from environment variables
        },
      }
    );
